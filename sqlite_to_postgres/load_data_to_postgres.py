...
from my_dataclasses import Person, Genre, Movie, MoviePerson, MovieGenre, random_date
...
class PostgresSaver:
    ...
    def load_movie(self, single_movie: dict) -> str:
        ...
        movie = Movie(title=single_movie.get("title"), description=single_movie.get("description"),
                      imdb_rating=single_movie.get("imdb_rating") or randrange(0, 10),
                      creation_date=random_date())
        data = (
            movie.id, movie.title, movie.movie_type, movie.file_path, movie.description, movie.imdb_rating,
            movie.rating, movie.creation_date, movie.age_limit, movie.created, movie.modified)
    ...
    def load_genre_and_movie_genre(self, single_movie, movie_id):
        ...
        for genre in genres:
            ...
            data = (_genre.id, _genre.name, _genre.description, _genre.created, _genre.modified)
            self.cursor.execute(f"""
                WITH query_sel AS (
    ...
    def load_person_and_movie_person(self, single_movie: dict, movie_id):
        ...
        for person in person_list:
            ...
            if not _person.firstname:
                continue
            data = (
                _person.id, _person.firstname, _person.lastname, _person.birthdate, _person.birthplace, _person.created,
                _person.modified)
            self.cursor.execute(f"""
                WITH query_sel AS (
            ...
            data = (
                movie_person.id, movie_person.movie_id, movie_person.person_id, movie_person.role, movie_person.created,
