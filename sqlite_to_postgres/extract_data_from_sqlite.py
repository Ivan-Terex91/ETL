...
class Movie:
    ...
    def __init__(self, cursor, idx, imdb_rating, genre, title, description, director, writer, writers):
        self.cursor = cursor
        self.id = idx
    ...
    def writer_list(self):
        ...
        if self.writers:
            ...
            writers_list = [writer for writer_sub_list in writers_list for writer in writer_sub_list]
        ...
        for writer in writers_list:
            self.cursor.execute(f"""SELECT writers.id, writers.name
                               FROM writers
    ...
    def person_list(self):
        ...
        actors = [(actor.get("name"), "actor") for actor in self.actors] if self.actors else [(None, "actor")]
        self.persons.extend(actors)
        writers = [(writer.get("name"), "writer") for writer in self.writers] if self.writers else [(None, "writer")]
